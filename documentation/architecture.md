...
| `src/components/shell/StickyMobileCTA.astro` | Fixed bottom bar visible on mobile/tablet (max-width 1023px) after the hero scrolls out of view. Slides up via `translateY` when `.is-visible` is toggled. Price label uses `cta.fromPrice` translation key (renders "around €100/night" — static placeholder, not derived from seasonal pricing). Action text uses `cta.checkAvailability`. Tapping navigates to `/{locale}/kontakt`. Visibility driven by a single `IntersectionObserver` on `.hero-sentinel`; bar shows when the sentinel is not intersecting, hides when it returns to view. No `lowestPrice` prop — price is hardcoded in the translation string. |
