...
- **Acceptance Criteria:**
  ...
  - **Homepage waves (3 placements):** (1) cream-to-dark before apartments section, (2) dark-to-cream after apartments section, (3) cream-to-sunset-gradient at the top of the sunset CTA section — each is an inline SVG embedded directly in the page template, with `fill="var(--color-bg)"` (cream) and `scaleY(-1)` vertical flip for wave-in placements, positioned absolute at the correct edge of the section. Sections receiving a wave at their top edge (apartments dark section, sunset CTA section) use the `.section--wave-in` CSS class for positioning and padding instead of inline styles. Each wave's `fill` color must exactly match the background of the section it transitions INTO. The `bg` (background) must match the section it sits ON TOP OF. No visible color gaps or mismatches between wave and adjacent sections.
