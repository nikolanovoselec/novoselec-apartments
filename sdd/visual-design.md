...
- **Acceptance Criteria:**
  ...
  - Section dividers: inline SVG waves embedded directly in page templates (no reusable component), always visible, `aria-hidden`, no scroll-triggered reveal
...
- **Applies To:** Visitor
- **Acceptance Criteria:**
  - **Inline SVG waves (no reusable component):** All wave dividers are inline `<svg>` elements embedded directly in page templates — there is no `WaveDivider.astro` component. Standard form: `viewBox="0 0 1440 80"`, `preserveAspectRatio="none"`, organic bezier curve path, `aria-hidden="true"`, positioned `absolute` at the top or bottom of a section, with a fluid height of `clamp(50px, 8vw, 100px)`. Two smaller variants exist for tighter contexts: HeroSimple uses `clamp(40px, 6vw, 80px)` for its bottom wave (REQ-VD-12), and the apartment listing's `.collage-strip` decorative top-edge wave uses an even smaller `viewBox="0 0 1440 60"` with a simpler 720-unit period bezier path at `clamp(30px, 5vw, 60px)` height to suit the strip's thinner band.
  - Fill color must match the target section background (e.g., dark navy for transition into dark section, cream for transition back to light)
  - Vertical mirroring via `transform: scaleY(-1)` for wave-in (top-of-section) placement vs the default wave-out (bottom-of-section) placement
  - **Homepage waves (3 placements):** (1) cream-to-dark before apartments section, (2) dark-to-cream after apartments section, (3) cream-to-sunset-gradient at the top of the sunset CTA section — each is an inline SVG embedded directly in the page template, with cream fill (waves 1-2 use literal `#F8F5EF`, wave 3 uses `var(--color-bg)` token) and `scaleY(-1)` vertical flip for wave-in placements, positioned absolute at the correct edge of the section. Sections receiving a wave at their top edge (apartments dark section, sunset CTA section) use the `.section--wave-in` CSS class for positioning and padding instead of inline styles. Each wave's `fill` color must exactly match the background of the section it transitions INTO. The `bg` (background) must match the section it sits ON TOP OF. No visible color gaps or mismatches between wave and adjacent sections.
  - **Homepage hero wave:** The homepage hero section (REQ-SF-1) includes an inline wave SVG at its bottom edge (cream fill `#F8F5EF`) for organic transition into the first content section. Part of the hero component's markup, not a separate abstraction.
  - **Subpage hero waves:** Every `HeroSimple` component with a background image includes an inline wave SVG at the bottom edge (cream fill `#F8F5EF`), creating an organic transition from hero photo into page content (REQ-VD-12). The HeroSimple component itself renders the inline wave SVG at its own bottom edge, so every page using HeroSimple gets the wave automatically; there are no pages using custom hero markup that would need to duplicate the wave manually. **Every page with a hero image MUST have a wave at the bottom of the hero.** No exceptions.
...
- **Acceptance Criteria:**
  ...
  - Wave SVG divider at bottom edge (same pattern as REQ-VD-9: `viewBox="0 0 1440 80"`, `preserveAspectRatio="none"`, organic bezier path, fill `#F8F5EF` to match page background, `aria-hidden="true"`). HeroSimple uses a slightly smaller responsive height `clamp(40px, 6vw, 80px)` than the full-section waves used on REQ-VD-9 section transitions (which use `clamp(50px, 8vw, 100px)`) to suit subpage hero scale.
...
- **Applies To:** Visitor
- **Acceptance Criteria:**
  - **Two component variants share the same marquee technique:**
    - **`ScrollCollage`** — used on the homepage and apartment listing page exterior photo strips. No `reverse` or `showCaptions` props. Default speed 35s. Renders only when at least one image is provided.
    - **`MiniCollage`** — used on editorial detail pages (vodic, dolazak, plaze, aktivnosti, hrana), apartment detail page interior strip, and the gallery page (galerija). Adds `reverse` and `showCaptions` props plus single-image fallback. Default speed prop is 20s; callers (gallery, editorial, apartment detail) override it explicitly to 35s+ via `Math.max(35, count * 8)` or a fixed 80s on galerija.
  - Pure CSS infinite horizontal scroll (marquee technique) — no JavaScript
  - `reverse` prop (MiniCollage only): when set, animation plays in reverse direction (`animation-direction: reverse`), scrolling right-to-left instead of the default left-to-right. Used for two-direction collage pairs (e.g., Food & Drink page, REQ-ED-8) and for alternating scroll direction on editorial pages (vodic, dolazak, plaze, aktivnosti — odd-indexed sections use `reverse`).
  - `showCaptions` prop (MiniCollage only, default false): when enabled, renders each photo's alt text as an overlay caption positioned absolute at the bottom of the photo. Caption styling: sans-serif (`var(--font-sans)`), base font size, normal weight (400), line-height 1.6, white text at 90% opacity, padding `var(--space-md) var(--space-lg)`, with a bottom-to-top gradient background (`rgba(0,0,0,0.5)` to transparent) for legibility over any image. `pointer-events: none` so the overlay does not interfere with hover pause. Currently enabled only on the gallery page (REQ-SF-8) to display poetic Croatian captions over photos.
  ...
  - Loop duration set per-call by the parent page: ScrollCollage default 35s; MiniCollage default 20s, overridden by callers to 35s+ (or 80s on the gallery page). `linear infinite` timing.
  ...
  - Placement matrix:
    ...
    - **Gallery page** (galerija, REQ-SF-8) — 142 photos split into strips of 10 with alternating scroll directions and deterministic locale-based shuffle order, fixed 80s duration per strip.
- **Constraints:** CON-PERF, CON-A11Y
- **Priority:** P2
