...
| **Turnstile** | Cloudflare's privacy-preserving CAPTCHA alternative. Used in managed mode (`data-appearance="always"`) on the inquiry form so the widget is visibly rendered to the visitor. |
...
| **Scroll Collage** | An infinitely scrolling horizontal photo band using pure CSS marquee technique (duplicated track with `translateX(-50%)` keyframe). The `ScrollCollage` component (no `reverse`/`showCaptions` props, default speed 35s) is used on the homepage and apartment listing page to showcase property exterior photos. Pauses on hover, respects reduced motion. See REQ-VD-15 for the placement matrix. |
