...
| **Image Resizing** | Cloudflare service that transforms images on-the-fly at the edge. Applied via `cf: { image: { ... } }` on Worker fetch responses from private R2 (through the `/api/img/{uuid}` route). Avoids Worker-side image processing. |
